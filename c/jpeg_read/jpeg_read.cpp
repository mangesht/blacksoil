...
class HuffmanLookup{
    ...
    */
    ...
    void decode(unsigned char *p, HuffmanLookup *huff[4], SOSMarker *sosMarker,QuantTblMarker *quantTbl) {
        ...
        printf("Quantization table index used = %d \n",qtblIdx);  
        if ((rxp>>16) == 0xFF00) {
        ...
        if ((rxp>>16) == 0xFF00) {
            ...
            if(huff[qtblIdx*2+1]->lkpt[rx].num <= 0) {
                ...
                if(val == 0) { 
                    ...
                    dcVal = dcVal * quantTbl->quantVal[qtblIdx][mcuIdx]; 
                    printf("abVal = %d ACVal = %d quantVal = %d \n",ab,dcVal,quantTbl->quantVal[qtblIdx][mcuIdx]);
                    mcu[mcuIdx++] = dcVal;
                    decodedCount++;
                }
            }
        }
        printf("MCU for component = %d \n",cc);
        display_8_8(mcu);
        }
    }
    ...
    int main(int argc,char **argv) {
        ...
        while(!end_reached) { 
            ...
            if(marker == SOI) {
                ...
                decode(buf,hfl,sosMarker,quantTbl); 
